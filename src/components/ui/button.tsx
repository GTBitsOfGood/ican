...
const Button: React.FC<ButtonProps> = ({ buttonType = 'store', drawButton = true }) => {
  ...
  const redirect = () => {router.push(`/${buttonType}`)}
  ...
  return (
    ...
    >
      ...
      {drawButton && (
        <>
          {/* Need to fix the logic for positioning these too */}
          <div 
            className="w-[230px] h-[120px] left-0 top-0 absolute bg-gradient-to-b 
            from-[#9ca1c9] via-[#676ca0] to-[#2f324d] 
            shadow-[0px_0px_0px_2px_rgba(61,112,201,0.40)] 
            shadow-[inset_0px_2px_1px_0px_rgba(0,0,0,0.25)] 
        ...
        <div className="w-full h-full z-10 flex flex-col items-center justify-end pb-[20px] gap-1">
          ...
          <div className="h-9 z-10 justify-center items-center inline-flex">
            <span className="font-quantico text-center text-white text-4xl font-bold leading-9 text-shadow-blue"> {name} </span>
          </div>
        </div>
      </div>
    </button>
